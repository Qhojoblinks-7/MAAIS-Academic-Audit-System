    ...
    <div className="flex-1 overflow-y-auto bg-[#F9F9F7] p-8 lg:p-12">
      <div className="max-w-6xl mx-auto">
        <header className="mb-10">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Fee ledger · procurement · income expenditure</p>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {[
            { label: 'Collected This Term', value: `GH₵${totalCollected.toLocaleString()}`, icon: Wallet, color: 'bg-emerald-50 text-emerald-700' },
            { label: 'Outstanding', value: `GH₵${totalOutstanding.toLocaleString()}`, icon: AlertTriangle, color: 'bg-rose-50 text-rose-700' },
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Fee Records — {feeRecords.length} Entries</span>
            </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {feeRecords.map((fee, i) => (
                  <tr key={fee.id} className="hover:bg-gray-50/50 transition-all">
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-gray-900 text-right">GH₵{fee.amount.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm font-black text-emerald-700 text-right">GH₵{fee.paid.toLocaleString()}</td>
