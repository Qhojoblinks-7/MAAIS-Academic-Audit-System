...
import {
  Scale, QrCode, CheckCircle2, Clock, ArrowRight, X, Star,
} from 'lucide-react';
    ...
    <div className="flex-1 overflow-y-auto bg-[#F9F9F7] p-6 md:p-8 lg:p-12">
      <div className="max-w-4xl mx-auto">
        <header className="mb-10">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Raise tickets Â· track requests Â· view resolution history</p>
        </header>
        ...
        <section className="bg-white rounded-[2rem] border border-gray-100 p-8 shadow-sm mb-8">
          <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex items-center gap-3 mb-6">
            ...
            <h2 className="text-[11px] font-black text-gray-900 uppercase tracking-widest">Observation Feed</h2>
          </div>
          <div className="space-y-4">
            {myObservations.map((obs, i) => (
              ...
              >
                ...
                <p className="text-xs font-medium text-gray-700 italic leading-relaxed">"{obs.comment}"</p>
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-2">Observed by {obs.teacher}</p>
              </motion.div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-3">
            {tickets.map((t) => (
              <div key={t.id} className="flex items-center justify-between p-4 rounded-2xl hover:bg-gray-50 transition-all">
                ...
                <span className={cn("text-[9px] font-black px-2 py-1 rounded uppercase tracking-widest border", statusStyles[t.status])}>{t.status}</span>
              </div>
            ))}
          </div>
        </section>
