...
import { ShieldCheck, Lock, Download, RefreshCw, AlertTriangle } from 'lucide-react';
...
export function HODCertification() {
  ...
  const [confirmingClass, setConfirmingClass] = useState(null); // Tracks the full class object safely
  ...
  const handleLock = async () => {
    ...
    // Fallback hierarchy to target the intended operational ID parameter
    const targetTermId = confirmingClass.termId || confirmingClass.id;
    setLockingId(confirmingClass.id);
    
    try {
      if (typeof lockTerm === 'function') {
        await lockTerm(targetTermId);
      }
    } finally {
      setLockingId(null);
  ...
  const canLock = (cls) => cls.submissionPct === 100 && cls.status !== 'LOCKED';
                        ...
                        <div className="w-full bg-gray-100/80 rounded-full h-1.5 overflow-hidden">
                          <div 
                            className={cn(
                              ...
                              cls.submissionPct === 100 ? "bg-emerald-500" : "bg-amber-500"
                            )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex gap-2 justify-end pt-2">
                ...
                <button
                  onClick={handleLock}
                  className="px-3.5 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold shadow-2xs transition-colors"
                >
                  Confirm Certification Lock
                </button>
              </div>
            </motion.div>
          </div>
