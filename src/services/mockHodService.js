...
const createMockService = () => ({
  getAuditLogs: async (params = {}) => {
    ...
    if (params.studentId) items = items.filter(i => i.studentId === params.studentId);
    if (params.teacherId) items = items.filter(i => i.userId === params.teacherId);
    if (params.subject) items = items.filter(i => i.subject === params.subject);
    if (params.startDate) items = items.filter(i => new Date(i.time) >= new Date(params.startDate));
    if (params.endDate) items = items.filter(i => new Date(i.time) <= new Date(params.endDate));
  ...
  getDepartmentProgress: async () => {
    await simulateDelay();
    return mockData.departmentProgress;
  },

  getTeacherSubmissionStatus: async () => {
    await simulateDelay();
    return mockData.teacherSubmissions;
  },

  getLockedTerms: async () => {
    await simulateDelay();
    return { items: mockData.lockedTerms.items };
  },
  ...
  unlockDepartmentMatrix: async (termId) => {
    ...
    return { success: true, message: 'Department matrix unlocked successfully' };
  },

  getGradeComparison: async (subjectId, termA, termB) => {
    await simulateDelay();
    return mockData.gradeComparison;
  },
  ...
  getArchivedDepartmentData: async (params = {}) => {
    ...
    if (params.promoted !== undefined) {
      items = items.filter(i => {
        const promo = mockData.promotions.items.find(p => p.studentId === i.id);
  ...
  getPromotionRecommendations: async (params = {}) => {
    await simulateDelay();
    return { items: mockData.promotions.items };
  },
  ...
  mfaEnroll: async () => {
    await simulateDelay();
    return { 
      qrCode: 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAwIiBoZWlnaHQ9IjIwMCI+PGNpcmNsZSBjeD0iMTAwIiByPSI4MCIgc3Ryb2tlPSIjMDBhIiBzdHJva2Utd2lkdGg9IjUiIGZpbGw9Im5vbmUiLz48L3N2Zz4=',
  ...
  getActiveSessions: async () => {
    await simulateDelay();
    return [
      { id: 'sess001', ip: '192.168.1.100', userAgent: 'Chrome on Windows', createdAt: '2026-05-22T08:00:00Z', current: true },
      { id: 'sess002', ip: '192.168.1.101', userAgent: 'Firefox on macOS', createdAt: '2026-05-21T14:00:00Z', current: false }
    ];
  },
  ...
  getDepartmentTeachers: async (params = {}) => {
    await simulateDelay();
    let items = [...mockData.teachers.items];
    if (params.search) {
      const q = params.search.toLowerCase();
  ...
  getInterventionAlertAggregation: async () => {
    ...
    alerts.forEach(a => {
      aggregation.bySubject[a.subject] = (aggregation.bySubject[a.subject] || 0) + 1;
      aggregation.byReason[a.reason] = (aggregation.byReason[a.reason] || 0) + 1;
    });
    return aggregation;
  },
  ...
  getJournalEditCaptures: async (params = {}) => {
    await simulateDelay();
    let items = mockData.auditLogs.items.filter(l => l.action === 'UPDATE' || l.action === 'AUTO_SAVE');
    if (params.studentId) items = items.filter(i => i.studentId === params.studentId);
    return { items };
  },
  ...
  validateLock: async (termId) => {
    ...
    return {
      ...
      isLocked: mockData.lockedTerms.items.find(t => t.id === termId)?.status === 'LOCKED',
      blockingIssues: pendingSubmissions > 0 ? [`${pendingSubmissions} pending submissions`] : [],
      warnings: [],
      pendingSubmissions
    };
  },
  ...
  exportWAECCSV: async (termId, className) => {
    ...
    const csv = ['Index,Student Name,SBA,Exam,Final,Grade,Roman']
      .concat(rows.map(r => `${r.index},${r.name},${r.sba},${r.exam},${r.final},${r.grade},`))
      .join('\r\n');
    return csv;
  },
  ...
  exportWAECPDF: async (termId) => {
    await simulateDelay();
    return { success: true, message: 'PDF exported', downloadUrl: `/exports/waec-${termId}.pdf` };
  }
});
