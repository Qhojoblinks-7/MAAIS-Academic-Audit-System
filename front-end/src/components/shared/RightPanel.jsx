...
const mappedRevisions = revs.slice(0, 3).map((rev) => ({
    ...
    fetchData();
  }, [setRightPanelVisible, user?.role]);
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {atRiskStudents.map((student) => (
            <div
              key={student.id}
              className="p-4 rounded-2xl bg-white border border-gray-100 flex items-center gap-4 hover:border-red-100 hover:bg-red-50/30 transition-all group"
            >
              <img
                src={student.avatar}
                alt={student.name}
